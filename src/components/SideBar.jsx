...
import { UserConfigSVG } from "../images/svg/UserConfigSVG";
...
export function SideBar() {
  ...
  return (
    <menu>
      <button
        onClick={openMenu}
        className="sm:hidden w-screen absolute p-4 bg-gray-900 text-white"
      >
        <MenuSVG />
      </button>
      <nav
        className={`flex justify-center w-60 h-screen py-10 border-r-2
         border-white bg-gray-900 overflow-y-hidden sm:translate-x-0 z-50 max-sm:absolute max-sm:${
           activeMenu ? "border-r-2" : "border-none"
         } ${activeMenu ? "-translate-x-60" : "translate-x-0"} duration-1000`}
      >
        <ul className="w-full overflow-y-auto">
          <button
            onClick={openMenu}
            className="sm:hidden w-full absolute p-4 top-0 bg-gray-900 text-white"
          >
            <CloseMenuSVG />
          </button>
          ...
          <Link to="/">
            <li
              onClick={openMenu}
              className="flex w-11/12 mx-auto my-2 items-center rounded-md py-2 px-5 gap-5 duration-100 hover:duration-100 hover:bg-gray-800"
            >
              <AgendTabSVG />
          ...
          <div className="group">
            ...
            <ul className="w-11/12 mx-auto max-h-0 easy ease-in-out duration-200 overflow-hidden group-hover:max-h-96 group-hover:duration-1000">
              <Link to="/CadastrarTurmas">
                <li
                  onClick={openMenu}
                  className="flex w-11/12 ml-auto my-2 items-center rounded-md py-2 px-5 gap-2 duration-100 hover:duration-100 hover:bg-gray-800"
                >
                  <CircleSVG />
                  <p className="font-normal text-white text-nowrap">
                    Cadastrar Turma
